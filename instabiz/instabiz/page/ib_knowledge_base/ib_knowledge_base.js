		...
		items: [
			...
			},
			{
				...
				steps: [
					...
					"Set <b>Priority</b> (Urgent/High/Normal/Low) and optional <b>Delivery Date</b> — new Sales Orders now default Delivery Date to 8 days out (see 2.9 in Sales and Quotations).",
				],
			...
			},
			{
				...
				updated: "2026-07-31",
				desc: "Workspace → Production Dashboard. Real-time factory floor overview. 4 KPI cards, a Location filter, location-scoped pipeline summary, priority strip, wastage card, AI actions panel, a searchable/paginated active plan table, recent entries.",
				link: "/app/ib-production-dashboard", linkLabel: "Open Production Dashboard",
				tags: "production dashboard kpi active pending completed today machines wastage ai actions plan entries pipeline priority strip location filter etd search pagination infinite scroll",
				steps: [
					...
					"<b>Location filter (new 2026-07-31):</b> dropdown — All Locations / Gujarat (Factory) / Maharashtra (Warehouse) / Chennai (Warehouse). Your choice is remembered in your browser (localStorage key <code>ib_prod_location</code>) and automatically carries over to the Production Stages page, and vice versa. See PROD-6.",
					"<b>Stage Pipeline:</b> 7 stage cards each showing Pending / In Progress / Completed counts + color progress bar. Click any card → Production Stages page. <b>Only renders once you've picked a specific location</b> (not \"All Locations\") — it always showed before, which didn't make sense for warehouse-only sites with just 2 stages.",
					...
					"<b>Active Production Plan Table:</b> one card per active order, identified by <b>Sales Order only</b> — the underlying Order Sheet record still exists and drives the click-through, but its raw ID is never shown, since it's an internal grouping record you don't need to think about. Each card shows customer, items, current stage, stage chip progress row, priority, overall progress bar, and a color-coded <b>ETD</b> badge (green = on track, orange = due within 2 days, red = overdue) based on the SO's Delivery Date — see 2.9 in Sales and Quotations for the new default ETD logic.",
					"<b>Search box (new)</b> above the Active Production Plan table — filters by Sales Order name or customer name.",
					"<b>Pagination (new):</b> loads 25 order sheets at a time; scrolling down the table loads more automatically (infinite scroll) — previously this table was hard-capped at 25 total with no way to see more.",
			...
			},
			{
				...
				updated: "2026-07-31",
				desc: "Click any Work Order row in Order-wise, Machine-wise, or Job Bundles to open the right-side detail panel. Shows full WO info, action buttons, and entry history. (Previously opened from the Pipeline tab, which was removed 2026-07-31.)",
				tags: "side panel work order assign machine start hold next stage move create delivery note link jumbo roll new entry buttons panel history order wise machine wise job bundles",
				steps: [
					...
					"<b>Next Stage →</b> (new) — visible when In Progress. Completes the current stage <i>and</i> auto-creates + assigns the next stage's Work Order to the least-loaded available machine for that stage, in one click.",
					"<b>Move to Stage</b> (new) — a manual stage-picker (choose any stage) + <b>Move</b> button, for correcting a mis-routed item. Auto-assigns the least-loaded available machine for the target stage.",
					"<b>Create Delivery Note</b> (new) — appears once the WO reaches <b>Completed</b> status at the <b>Ready to Deliver</b> stage. Creates a Delivery Note scoped to just that single item, not the whole Sales Order, via <code>instabiz.overrides.sales_order.custom_make_delivery_note</code> with a new <code>item_code</code> parameter — useful when other items on the same order aren't ready yet.",
				...
				note: "Panel closes with Esc. Completing or moving a WO from the panel auto-refreshes the tab you're viewing.",
			},
			{
			...
			},
			{
				...
				updated: "2026-07-31",
				desc: "Production Stages → Order-wise tab — now the <b>default tab</b> on the page, since the Kanban/Pipeline tab was removed 2026-07-31 (see PROD-6). Lists all Order Sheets with filters and search. Click into one for three detail subtabs.",
				tags: "order wise tab sheet list filter status priority subtab matrix product stage machine create wo new order sheet default search location filter",
				steps: [
					"Click the <b>Order-wise</b> tab (or just land here — it's the default now).",
					...
					"<b>Search box (new):</b> filters the table by Sales Order name or customer name.",
					"<b>Table columns:</b> Sales Order, Customer, Items count, Progress bar, Priority badge, Status, View button — the raw Order Sheet ID isn't shown as a column; the Order Sheet record still exists underneath and drives the click-through, you just never need to look at its name.",
					"<b>+ New Order Sheet button</b> — opens dialog: Sales Order (link), Priority, Notes. Creates Order Sheet and auto-generates WOs (route depends on location — see PROD-1).",
					"<b>Click View / a row</b> → Order Sheet detail with 3 subtabs:",
					"  → <b>Order-wise subtab:</b> items list with their Work Orders per stage (WO name, status, machine, progress). Click a Work Order row here to open the side panel (see PROD-7) with Next Stage / Move / Create Delivery Note actions.",
			...
			},
			{
				...
				steps: [
					...
					"Open a Coating or Slitting WO row (Order-wise, Machine-wise, or Job Bundles) → side panel → click <b>Link Jumbo Roll</b>.",
			...
			},
			{
				...
				steps: [
					...
					"<b>2.</b> Production team opens SO → clicks <b>Create Order Sheet</b> (or it's auto-created within moments of SO submit — see PROD-16).",
