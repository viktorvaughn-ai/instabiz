		...
		items: [
			...
			},
			{
				...
				steps: [
					...
					"<b>7. Delivered</b> — Goods dispatched against Delivery Note. Exists in the stage model (colors) but no item-group or location route auto-creates a Work Order here — see PROD-1. Reachable only via the manual stage-picker + Move button in the Work Order side panel (see PROD-7); dispatch itself is still tracked independently via the Delivery Note / SO dispatch badge, not by this stage.",
				],
				updated: "2026-07-31",
			},
			{
			...
			},
			{
				...
				updated: "2026-08-03",
				desc: "Click any Work Order row in Order-wise, Machine-wise, or Job Bundles to open the right-side detail panel. Shows full WO info, action buttons, and entry history.",
				tags: "side panel work order assign machine start hold next stage move create delivery note new entry buttons panel history order wise machine wise job bundles print job order adjust qty",
				steps: [
					...
					"<b>Start</b> — visible when status = Pending or On Hold (Resume). Sets In Progress, records started_at.",
					"<b>On Hold</b> — visible when In Progress. Pauses WO (e.g. machine breakdown).",
					"<b>Next Stage →</b> — visible when In Progress. Completes the current stage <i>and</i> auto-creates + assigns the next stage's Work Order to the least-loaded available machine for that stage, in one click.",
					"<b>Move to Stage</b> — a manual stage-picker (choose any stage in the item's own route) + <b>Move</b> button, for correcting a mis-routed item. Activates the pre-existing target-stage Work Order and cancels the one being moved from; auto-assigns the least-loaded available machine for the target stage.",
					"<b>Adjust Qty</b> — Factory Management/System Manager only. Sets <code>pcs_to_make</code>/<code>logs_to_make</code> — manager-set reconciliation values shown on the printed Job Order (see PROD-26), separate from target_qty.",
					"<b>Print</b> — prints the <b>IB Job Order</b> format for this single Work Order (item/stage/machine/dimensions/qty, no customer or SO info). See PROD-26.",
					"<b>Create Delivery Note</b> — appears once the WO reaches <b>Completed</b> status at the <b>Ready to Deliver</b> stage. Creates a Delivery Note scoped to just that single item, not the whole Sales Order, via <code>instabiz.overrides.sales_order.custom_make_delivery_note</code> with an <code>item_code</code> parameter — useful when other items on the same order aren't ready yet.",
				...
				note: "Panel closes with Esc. Completing or moving a WO from the panel auto-refreshes the tab you're viewing. The old <b>Link Jumbo Roll</b> button (Coating/Slitting WOs) was removed from this panel 2026-07-31 — confirmed confusing and never actually used (0 real links ever). The backend function still exists, dormant; Jumbo Roll batch lineage is otherwise tracked via the Item-wise tab (PROD-9) and IB Jumbo Roll doctype directly (PROD-13).",
			},
			{
				...
				steps: [
					"Open a WO row (Order-wise, Machine-wise, or Job Bundles) → side panel → click <b>+ New Entry</b>.",
	...
	},
	{
		...
		items: [
			...
			},
			{
				...
				desc: "6 tabs — Sales, Inventory, Production, HR, Finance, Me — each showing KPIs + a trend chart + a breakdown. Every tab is open to every role from any of the 6 role-specific workspaces; what you see inside each tab depends on your role, not which tabs you can click.",
				link: "/app/ib-analytics-hub", linkLabel: "Analytics Hub",
				tags: "analytics hub cross module sales inventory production hr finance me content aware scoped role purchase stock",
				steps: [
					...
					"<b>Finance</b> tab — privileged: System Manager, Sales Manager, Accounts Manager, Factory Management, Purchase Manager. Everyone else sees their own customers' outstanding AR and collections only — no AP, no other reps' customers.",
					"<b>Me</b> tab — unchanged, always self-scoped for everyone regardless of role.",
					"Daily / Weekly / Monthly period selector and the CSV export both work the same regardless of which view (scoped or company-wide) you're looking at.",
				],
				tip: "If a number looks unexpectedly small on Sales/Inventory/Production/HR/Finance, check whether that tab is your role's domain — a non-privileged-for-that-tab user is intentionally only seeing their own work, not a bug. Stock User and Purchase User (non-Manager) are deliberately left scoped everywhere, same as any other operational role.",
			},
		],
	},
	{
