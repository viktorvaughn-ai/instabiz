...
const KB_SECTIONS = [
	...
	},
	{
		...
		items: [
			{
				num: "AI.0", title: "AI Agent System — How It Works (23 Agents)",
				updated: "2026-08-03",
				desc: "23 agents run daily across Sales, Production, HR, Finance, and Operations. Each agent reads live Frappe data, builds a deterministic action draft, and queues it as IB AI Action (status=pending). No action executes until a human approves it in the AI Inbox. Claude API optionally phrases the human-facing summary in natural language.",
				link: "/app/ib-ai-inbox", linkLabel: "Open AI Inbox",
				tags: "ai agent system overview 23 agents human in the loop approve reject claude llm deterministic ib ai action dedup daily schedule",
				steps: [
			...
			},
			{
				...
				desc: "Trigger all 23 agents immediately from the AI Inbox page, or run a single agent by code. Useful for testing or when you need fresh suggestions without waiting for the midnight scheduler.",
				...
				steps: [
					...
					"All 23 agents run immediately (trigger_type = 'manual'). New pending actions appear — refresh to see.",
			...
			},
			{
				...
				steps: [
					...
					"<b>istix_enforcer</b> — IB Work Orders stalled In Progress 8+ hours → escalates to Manufacturing Manager. On Approve: sends bell notification with WO name, stage, and hours stalled.",
			...
			},
			{
				...
				steps: [
					...
					"<b>hr_late_checkin</b> — Employees with 5+ late arrivals this month → flags to HR Manager. On Approve: sends bell with count.",
				],
			},
			{
				...
				steps: [
					...
					"<b>finance_expense_pending</b> — Expense Claims submitted but not yet approved for 3+ days → nudges the approver to review.",
			...
			},
			{
				...
				steps: [
					...
					"<b>ops_stock_aging</b> — Items with no bin movement in 60+ days → flags to Warehouse Manager for write-off or discount review. On Approve: sends bell with item list.",
				],
			},
			{
				num: "AI.C1", title: "Copilot — Ask Business Questions in Plain Language (New, 2026-08-10)",
				updated: "2026-08-10",
				desc: "Different from the AI Agents above: Copilot doesn't draft actions for approval, it just answers direct questions — overdue customers, top-selling items, low stock, production status, sales summary, AR aging, pending leave approvals — by picking from a fixed set of pre-written, permission-scoped queries and phrasing the answer in natural language.",
				link: "/app/ib-copilot", linkLabel: "Open Copilot",
				tags: "copilot ask question chat natural language overdue top items low stock production status sales summary ar aging pending leaves",
				steps: [
					"Go to <b>Workspace → Copilot</b> (visible across Sales, Finance, Procurement, Stock, Production, HR workspaces depending on your role).",
					"Type a question in plain language, e.g. \"which customers are overdue\" or \"what's low on stock\".",
					"Copilot matches your question to one of its known question types and pulls live data — never generates its own database query, so it can't answer arbitrary open-ended questions outside that fixed list.",
					"Answers are scoped to what you're allowed to see — a Sales User asking about overdue customers only sees their own customers, same as everywhere else in the app.",
					"Suggestion chips under the question box show what it can currently answer for your role.",
				],
				note: "If Copilot can't match your question, it says so and lists what it can answer — it never guesses or fabricates a query.",
			},
			{
				num: "AI.D1", title: "Document Intake — Extract a Draft Order from Pasted Text (New, 2026-08-10)",
				updated: "2026-08-10",
				desc: "Paste a raw PO or SO email/text (from a customer or supplier), AI extracts items/quantities/party, you review the extraction, then convert it into a real draft document once it looks right. The AI never creates a document by itself — it only fills out the review record; a human always confirms the conversion.",
				link: "/app/ib-document-intake", linkLabel: "Document Intake",
				tags: "document intake ai extraction paste text po so email draft convert review matched party",
				steps: [
					"Create a new <b>IB Document Intake</b> record, choose Sales Order or Purchase Order, paste the raw text.",
					"Click <b>Extract</b> — AI reads the text and fills in item lines, quantities, and tries to match the customer/supplier by name.",
				...
				note: "If the AI extraction fails (no API credits, unreadable text, etc.) the record just shows an extraction error and stays in Draft — nothing is silently created.",
			},
		],
	},
	{
	...
	},
	{
		...
		items: [
			...
			},
			{
				...
				note: "No item group route ever includes \"Delivered\" — every route ends at Ready to Deliver, so a Work Order is never auto-created at the Delivered stage (it can only be reached via the manual stage-picker + Move button in the Work Order side panel — see PROD-7; the old Pipeline/Kanban drag-and-drop view was removed 2026-07-31). Stages that don't apply to an item group are simply skipped — no WOs are created for them. Also: item_group \"BOPP\" is NOT in the Gujarat routing map despite being a real item_group used elsewhere in the app (e.g. batch tracking) — BOPP items on Gujarat orders silently fall back to the 3-stage default route (Cutting → Packing → RTD) rather than a coating/slitting route. Flagged as an unconfirmed gap, not fixed in code — check with Factory Management whether BOPP needs its own route.",
			},
			{
				...
				steps: [
					...
					"<b>7. Delivered</b> — Goods dispatched against Delivery Note. As of 2026-08-11: submitting a Delivery Note automatically moves that item's Work Order from Ready to Deliver to Delivered — no manual step needed. No item-group or location route ever auto-creates a Work Order directly at this stage (see PROD-1); it's only ever reached this way, or via the manual stage-picker + Move button in the Work Order side panel (see PROD-7) for an out-of-sequence correction.",
				],
				updated: "2026-07-31",
			},
			{
			...
			},
			{
				...
				tags: "work order status lifecycle pending in progress on hold completed fields stage machine operator qty started completed advance next naming",
				steps: [
					...
					"<b>Completed</b> — Click Complete (or Next Stage →, which completes and advances in one click). completed_qty defaults to target_qty since per-stage partial output isn't captured anywhere (IB Production Entry is unused by design — see the FAQ). System auto-creates the next-stage WO and activates it.",
					"Completing the last applicable stage (per item group + location routing) marks the entire item as production-complete.",
			...
			},
			{
				...
				updated: "2026-08-05",
				desc: "Click any Work Order row in Order-wise, Item-wise, Stage-wise, Machine-wise, or Job Bundles to open the right-side detail panel. Rebuilt 2026-08-05: one contextual primary button (its label changes with WO state) plus a dropdown (▾) for the less-frequent actions — replacing the old row of 5-9 equal-weight buttons.",
				tags: "side panel work order assign machine start resume hold next stage move create delivery note buttons panel dropdown order wise machine wise stage wise job bundles print job order adjust qty",
				steps: [
					...
					"<b>Dropdown (▾) — less-frequent actions:</b> <b>Put On Hold</b> (valid from Pending or In Progress — pauses the WO, e.g. machine breakdown), <b>Print Job Order</b> (prints the <b>IB Job Order</b> format for this single WO — item/stage/machine/dimensions/qty, no customer or SO info, see PROD-26), <b>Adjust Qty</b> (Factory Management/System Manager only — sets <code>pcs_to_make</code>/<code>logs_to_make</code>, manager-set reconciliation values shown on the printed Job Order, separate from target_qty; dialog and panel header show the item's UOM and target qty explicitly as of 2026-08-10, and the wrong field for the WO's UOM is now rejected instead of silently accepted), <b>Move to Different Stage</b> (manual stage-picker, hidden behind a small icon toggle — choose any stage, not just the item's own route as of 2026-08-05, see PROD-27; Gujarat can reach all 7 stages, Maharashtra/Chennai stay restricted to Packing→Ready to Deliver; activates the target stage's existing WO or creates one on the fly, cancels the WO being moved from).",
				],
				note: "Panel widened 400px → 440px in the 2026-08-05 rebuild. Closes with Esc. Completing or moving a WO auto-refreshes the tab you're viewing. <b>\"+ New Entry\" / Entries History were removed from this panel 2026-08-05</b> — see PROD-8, that feature never had real data behind it. The old <b>Link Jumbo Roll</b> button (Coating/Slitting WOs) was removed 2026-07-31 — confirmed confusing and never actually used (0 real links ever); backend function still exists, dormant. Jumbo Roll batch lineage is tracked via the Item-wise tab (PROD-9) and IB Jumbo Roll doctype directly (PROD-13).",
			},
			{
			...
			},
			{
				...
				steps: [
					...
					"<b>Print button</b> — prints the <b>IB Job Order Summary</b> format: one landscape page listing every item's current stage in a stage×machine grid (see PROD-26). Only each item's current non-Completed stage is included, one Work Order per item — not every historical/future stage.",
			...
			},
			{
				...
				note: "Machine type must match stage — only Coating machines appear in the Coating WO Assign Machine picker. \"Despatch\" is the machine_type used for both Ready to Deliver and Delivered stage assignment (see _STAGE_MACHINE_TYPE in production.py). \"Quality Control\" is a valid machine_type Select option but currently unused — no stage maps to it.",
			},
			{
				...
				steps: [
					...
					"<b>Search box (new):</b> type an item code to filter bundles.",
					...
					"<b>Bundle table rows:</b> WO name, Sales Order, Customer, target qty, Priority badge, batch_group label. Click a row to open the Work Order side panel (see PROD-7).",
				...
				note: "Fixed 2026-07-31: a bundle you'd already run Batch Assign on used to reappear identically on refresh (the underlying query only checked status='Pending', not whether a machine was already assigned). Bundles now correctly disappear once machine-assigned.",
			},
			{
			...
			},
			{
				...
				steps: [
					...
					"<b>3.</b> Order Sheet created. Priority and WOs auto-assigned based on item group and location (Gujarat = full route; Maharashtra/Chennai = Packing → RTD only — see PROD-1).",
					"<b>4.</b> Open <b>Production Stages → Order-wise</b> (default tab). WOs appear under their Order Sheet at the first applicable stage; use the Location filter to scope to one site (see PROD-6).",
					"<b>5.</b> Click <b>Assign Machine</b> in the side panel for each WO. Or use Job Bundles tab to batch-assign.",
					"<b>6.</b> Click <b>Start</b> on the WO.",
					"<b>7.</b> Log production entries during the shift (<b>+ New Entry</b> in the side panel).",
					"<b>8.</b> Click <b>Next Stage →</b> (completes the current stage and activates the next one in a single click) or <b>Complete</b> alone if this is the item's last stage.",
					"<b>9.</b> Repeat Start → Log → Next Stage for each subsequent stage.",
					"<b>10.</b> After the last Packing WO completes → WO moves to Ready to Deliver. Bell notification sent to sales person (also at 25/50/75% milestones along the way — see PROD-25).",
					"<b>11.</b> Print a floor copy any time via the WO side panel's <b>Print</b> button (IB Job Order) or the Order-wise row's Print button (IB Job Order Summary) — see PROD-26.",
					"<b>12.</b> Sales creates Delivery Note from the SO. Production badge on SO → Dispatched → Delivered.",
					"<b>13.</b> End-of-day: review <b>DPR</b> (Daily mode) for wastage vs. norm per stage and machine.",
				],
			},
			{
			...
			},
			{
				...
				steps: [
					"<b>IB Job Order</b> (doctype IB Work Order, its default_print_format) — one full page per Work Order: item code/name/stage/machine, dimensions (joined from the matching Sales Order Item row by item_code — the customer/SO name itself is never shown), qty + UOM, conditional Pieces-to-Make/Logs-to-Make (see Adjust Qty below), Jumbo Roll size, signature footer. Print from the WO side panel's <b>Print</b> button (see PROD-7).",
					"<b>IB Job Order Summary</b> (doctype IB Order Sheet, its default_print_format, landscape) — a single compact page per order listing every item's current stage in one stage×machine grid (done/current/pending/not-in-route all visually distinct). Print from the Order-wise tab's <b>Print</b> (printer icon) button (see PROD-10). Only each item's current non-Completed stage is included — one Work Order per item, not a multi-page dump of every historical/future stage.",
		...
		items: [
			...
			},
			{
				...
				steps: [
					...
					"Everything the Pipeline tab could do (start/hold/complete/move a stage) is still available — click a Work Order row in Order-wise, Machine-wise, or Job Bundles to open the same side panel, which now also has Next Stage →, a manual stage-picker + Move, and Create Delivery Note.",
		...
		items: [
			{
				...
				updated: "2026-07-31",
				desc: "All 28 custom pages live under <code>instabiz/instabiz/page/&lt;name&gt;/</code> (each has a matching <code>.py</code> + <code>.js</code>). Shared cross-cutting overrides live in <code>instabiz/overrides/</code>. Use this as a quick index — search this Knowledge Base by topic for the how-to, use this table for the where.",
