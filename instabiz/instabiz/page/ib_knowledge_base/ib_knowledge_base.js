...
const KB_SECTIONS = [
	...
	},
	{
		...
		items: [
			...
			{
				num: "PROD-12", title: "Job Bundles — Removed 2026-08-13",
				updated: "2026-08-13",
				desc: "The Job Bundles tab (batch-assign a machine to several same-item-same-stage Work Orders at once) was removed. Its candidate pool — Pending Work Orders with no machine yet — can no longer exist: the Just-In-Time stage picker (see PROD-4) always auto-assigns a machine and starts a Work Order in the same action, so nothing ever sits Pending-and-unassigned anymore.",
				tags: "job bundles tab batch assign machine removed pending work orders same item stage group",
				steps: [
					"There is no Job Bundles tab anymore. The Stages tab's sub-tabs are Order-wise (default), Item-wise, Stage-wise, Machine-wise.",
					"To run several same-item orders on one machine, start each one via the Start Production picker (PROD-4) and pick the same machine when Assign Machine offers a choice — there's no batch-assign shortcut for this anymore.",
				],
				note: "The batch_assign_machine backend function itself is still there and still reachable via the \"Bundle Jobs\" AI agent suggestion (see PROD-20-ish AI Agents section) — that agent's own candidate query hits the same \"nothing is ever Pending-unassigned\" wall, so it will realistically never find anything to suggest either. Not touched — a separate feature with its own scheduler wiring.",
			},
			{
